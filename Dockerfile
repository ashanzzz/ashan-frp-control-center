...
RUN apt-get update \
    && apt-get install -y --no-install-recommends ca-certificates tini wget tzdata \
    && rm -rf /var/lib/apt/lists/*

WORKDIR /app
COPY .release/ashan-frp-server /app/ashan-frp-server
COPY .release/web/ /app/web/
RUN mkdir -p /data/frpc /data/logs && chmod 0755 /app/ashan-frp-server
...
ENTRYPOINT ["/usr/bin/tini", "--", "/app/ashan-frp-server"]
